...
extends PrimitiveSpreadBuilder<float[]> {
    ...
    @Override
    protected int getSize(@NotNull float[] arrf) {
        Intrinsics.checkNotNullParameter(arrf, "$this$getSize");
        return arrf.length;
    }

    public final void add(float f2) {
        FloatSpreadBuilder floatSpreadBuilder = this;
        int n2 = floatSpreadBuilder.getPosition();
        floatSpreadBuilder.setPosition(n2 + 1);
        this.values[n2] = f2;
    }
    ...
    public FloatSpreadBuilder(int n2) {
        super(n2);
